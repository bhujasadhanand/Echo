...
export default function App() {
  ...
  const applyTelemetry = useCallback((items) => {
    const nodes = {};
    ...
    for (const item of items) {
      ...
      const node =
        typeof item.node === "string"
          ? item.node.trim().toUpperCase()
          : undefined;
      if (node === "A" || node === "B" || node === "C") {
        nodes[node] = {
          fillLevel: Math.max(0, Math.min(100, Math.round(fill))),
    ...
    if (updates.size > 0) {
      setBins((prev) =>
        prev.map((bin) => {
          ...
          const fill = Math.max(
            0,
            Math.min(100, Math.round(Number(u.fillLevel))),
          );
          const status = fill <= 30 ? "empty" : fill <= 80 ? "half" : "full";
          return {
            ...bin,
            fillLevel: fill,
            status,
