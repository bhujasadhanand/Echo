...
export function StatusCards({ bins = [] }: { bins?: Bin[] }) {
  ...
  const cards = [
    {
      ...
      icon: Trash2, bgColor: 'bg-green-50', iconColor: 'bg-green-600', textColor: 'text-green-600', trend: 'down',
    },
    {
      title: 'Half-Filled Bins',
      count: halfBins,
      percentage: Math.round((halfBins / totalBins) * 100),
      icon: Trash2, bgColor: 'bg-yellow-50', iconColor: 'bg-yellow-600', textColor: 'text-yellow-600', trend: 'neutral',
    },
    {
      title: 'Completely Filled Bins',
      count: fullBins,
      percentage: Math.round((fullBins / totalBins) * 100),
      icon: Trash2, bgColor: 'bg-red-50', iconColor: 'bg-red-600', textColor: 'text-red-600', trend: 'up',
    },
  ];
            ...
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                {card.trend === 'up' ? (
                  <div className="flex items-center gap-1 text-red-600 text-sm">
                    <TrendingUp className="size-4" /><span>Alert</span>
                  </div>
                ) : card.trend === 'down' ? (
                  <div className="flex items-center gap-1 text-green-600 text-sm">
                    <TrendingDown className="size-4" /><span>Good</span>
                  </div>
                ) : null}
              </div>
              <div className="space-y-2">
                ...
                <div className="flex items-end gap-2">
                  <div className={`${card.textColor} text-3xl font-semibold`}>{card.count}</div>
                  <div className="text-gray-500 text-sm mb-1">({card.percentage}%)</div>
                </div>
