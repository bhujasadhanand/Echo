...
export function Analytics({ bins = [] }: AnalyticsProps) {
  const fullBins   = useMemo(() => bins.filter((b) => b.status === 'full').length,  [bins]);
  const halfBins   = useMemo(() => bins.filter((b) => b.status === 'half').length,  [bins]);
  const emptyBins  = useMemo(() => bins.filter((b) => b.status === 'empty').length, [bins]);
  ...
  const metrics = [
    { label: 'Today Collected', value: totalBins - fullBins, icon: CheckCircle2, bg: 'bg-blue-100', color: 'text-blue-600' },
    { label: 'Pending',         value: fullBins,             icon: Clock,        bg: 'bg-orange-100', color: 'text-orange-600' },
    { label: 'Avg Response',    value: '24 mins',            icon: TrendingUp,   bg: 'bg-green-100', color: 'text-green-600' },
    { label: 'Efficiency',      value: `${efficiency}%`,     icon: Target,       bg: 'bg-purple-100', color: 'text-purple-600' },
  ];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="font-semibold text-gray-900 mb-4">Bins Filled Per Day</h3>
        ...
        {/* Pie Chart */}
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <h3 className="font-semibold text-gray-900 mb-4">Fill Level Distribution</h3>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
          ...
          <div className="flex-1">
            ...
            <div className="mt-3 flex gap-3">
              ...
              <div className="text-sm">
                ...
                <span className="font-medium text-blue-600 ml-1">{totalBins - fullBins}/{totalBins}</span>
              </div>
            </div>
          </div>
        </div>
