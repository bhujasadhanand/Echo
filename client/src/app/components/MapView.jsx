...
export function MapView({ bins = [] }) {
  ...
  const rightAngleRoutePoints = (() => {
    ...
    const points = [fullBinRoutePoints[0]];
    ...
    <Card>
      ...
      <CardContent>
        ...
        >
          ...
          <div className="absolute top-1/2 left-4 text-xs text-gray-500 font-medium -rotate-90">
            West Zone
          </div>
          <div className="absolute top-1/2 right-4 text-xs text-gray-500 font-medium rotate-90">
            East Zone
          </div>
          ...
          {bins.map((bin, index) => {
            ...
            const isVisible =
              ...
              (!selectedBinId && hoveredBinId === bin.id);
            const preferBelow = pos.y < 22;
            const preferLeft = pos.x < 22;
            const preferRight = pos.x > 78;
            const popupVerticalClass = preferBelow
              ? "top-full mt-2"
              : "bottom-full mb-2";
            const popupAlignClass = preferLeft
              ? "left-0"
              : preferRight
                ? "right-0 left-auto"
            ...
            return (
              <div
                ...
                onMouseEnter={() => {
                  if (!selectedBinId) setHoveredBinId(bin.id);
                }}
                onMouseLeave={() => {
                  if (!selectedBinId) setHoveredBinId(null);
                }}
                onClick={(e) => {
                  e.stopPropagation();
                  setHoveredBinId(null);
              ...
              >
                <div className="relative">
                  <div
                    className={`w-8 h-8 ${getStatusColor(bin.status)} rounded-full border-4 border-white shadow-lg transform transition-transform hover:scale-125 flex items-center justify-center`}
                  >
                  ...
                  >
                    <div className="bg-white rounded-lg shadow-xl border border-gray-200 p-3 min-w-[220px]">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            <div className="flex-1 bg-gray-200 rounded-full h-2">
                              <div
                                className={`h-full rounded-full ${getStatusColor(bin.status)}`}
                        ...
                        <Badge
                          className={`${getBadgeColor(bin.status)} text-xs`}
                        >
                          {bin.status === "full"
                            ? "Full"
                            : bin.status === "half"
          ...
          <div className="absolute top-4 right-4 bg-white rounded-lg shadow-md p-3 border border-gray-200">
            <div className="relative w-12 h-12">
              ...
              <div className="absolute top-0 left-1/2 -translate-x-1/2 text-xs font-semibold text-gray-700">
                N
              </div>
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 text-xs font-semibold text-gray-400">
                S
              </div>
              <div className="absolute left-0 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400">
                W
              </div>
              <div className="absolute right-0 top-1/2 -translate-y-1/2 text-xs font-semibold text-gray-400">
                E
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center gap-6 mt-4 px-2">
          {[
            ["bg-green-500", "Empty (0–30%)"],
            ["bg-yellow-500", "Half Filled (31–80%)"],
            ["bg-red-500", "Full (81–100%)"],
          ].map(([color, label]) => (
