...
export function BinList({ bins = [] }: { bins?: Bin[] }) {
                ...
                <TableRow key={bin.id} className="hover:bg-gray-50">
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2 min-w-[180px]">
                      <Progress value={bin.fillLevel} className="flex-1 bg-gray-300 [&>div]:bg-black" />
                      <span className="text-sm text-gray-600 min-w-[40px]">{bin.fillLevel}%</span>
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge className={getStatusColor(bin.status)}>{getStatusLabel(bin.status)}</Badge>
                  </TableCell>
                  <TableCell>
