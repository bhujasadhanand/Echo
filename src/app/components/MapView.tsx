...
export function MapView({ bins = defaultBins }: { bins?: Bin[] }) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        >
          ...
          {/* Bin markers positioned across the map */}
          {bins.map((bin, index) => {
            const pos = markerPositions[index] || { x: 50, y: 50 };
            const position = { top: `${pos.y}%`, left: `${pos.x}%` };
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            try {
                              sessionStorage.setItem('workerAssignment.preselectBinId', bin.id);
                            } catch {
                              // ignore
                            }
                            setSelectedBinId(null);
        ...
        <div className="flex items-center gap-6 mt-4 px-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">Empty (0-30%)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-yellow-500" />
            <span className="text-sm text-gray-600">Half Filled (31-80%)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-3 h-3 rounded-full bg-red-500" />
            <span className="text-sm text-gray-600">Full (81-100%)</span>
          </div>
        </div>
      </CardContent>
    </Card>
