...
import { workers, bins as defaultBins, type Bin } from '../data/mockData';

export function WorkerAssignment({ bins = defaultBins }: { bins?: Bin[] }) {
  ...
  const selectedBins = useMemo(() => {
    return fullBins.filter((b) => selectedBinIds.has(b.id));
            ...
            {workers.map((worker) => {
              ...
              return (
                <div
                  ...
                  onClick={() => {
                    setSelectedWorkerId((prev) => (prev === worker.id ? undefined : worker.id));
                  }}
                >
        ...
        <div className="flex gap-3">
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (selectedBins.length === 0) {
                toast('Select bins to preview a route');
                return;
              }
              toast(`Route preview for ${selectedBins.length} selected bin(s)`);
            }}
          >
            View Route
          </Button>
