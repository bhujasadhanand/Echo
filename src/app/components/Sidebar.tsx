import { BarChart3, ChevronLeft, ChevronRight, History, LayoutDashboard, MapPin, Trash2, Users } from 'lucide-react';
...
export function Sidebar({ onNavigate }: SidebarProps) {
  ...
  return (
    ...
    >
      <div className={`${collapsed ? 'p-3' : 'p-6'} flex items-center justify-between`}
      >
        <div className={`text-white text-sm font-medium ${collapsed ? 'hidden' : 'block'}`}>Menu</div>
        <button
          type="button"
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`
                w-full flex items-center ${collapsed ? 'justify-center' : 'gap-3'} ${collapsed ? 'px-2' : 'px-3'} py-3 mb-1 rounded-lg transition-colors
                ${isActive 
